...
- (instancetype)initWithSize:(CGSize)size
{
    if (self = [super initWithSize:size])
    {
        ...
        SKSpriteNode *floorPlan = [SKSpriteNode spriteNodeWithImageNamed:@"Afterhours_zoom"];
        
        SKLabelNode *label = [SKLabelNode labelNodeWithText:@"Afterhours"];
        SKLabelNode *levelLbl = [SKLabelNode labelNodeWithText:@"Level 1"];
        
        floorPlan.position = CGPointMake(CGRectGetMidX(self.frame), CGRectGetMidY(self.frame) + 10);
        
        [self addChild:floorPlan];
        
        levelLbl.fontSize = 22;
        levelLbl.fontName = @"HelveticaNeue-Thin";
        levelLbl.position = CGPointMake(CGRectGetMidX(self.frame), CGRectGetMinY(floorPlan.frame) - 40);
